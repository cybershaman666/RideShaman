...
export const TariffSettingsModal: React.FC<TariffSettingsModalProps> = ({ initialTariff, onSave, onClose }) => {
  ...
  const handleReset = () => {
    if (window.confirm("Opravdu chcete obnovit výchozí nastavení tarifu?")) {
    ...
    <div className="fixed inset-0 bg-black/70 z-50 flex justify-center items-center p-4 animate-fade-in" role="dialog" aria-modal="true">
      <div className="bg-slate-800 rounded-lg shadow-2xl w-full max-w-lg relative">
        <div className="flex justify-between items-center p-6 border-b border-slate-700">
          <h2 className="text-xl font-semibold">Nastavení Tarifu</h2>
        ...
        <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
            <section>
                <h3 className="text-lg font-medium text-amber-400 mb-3">Sazby za jízdu</h3>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <NumberInput label="Nástupní sazba" id="startingFee" value={tariff.startingFee} onChange={handleChange} />
                    <NumberInput label="Cena/km (osobák)" id="pricePerKmCar" value={tariff.pricePerKmCar} onChange={handleChange} />
                    <NumberInput label="Cena/km (dodávka)" id="pricePerKmVan" value={tariff.pricePerKmVan} onChange={handleChange} />
                </div>
            </section>
            
            <section>
                <div className="flex justify-between items-center mb-3">
                    <h3 className="text-lg font-medium text-amber-400">Paušální sazby</h3>
                    <button onClick={addFlatRate} className="flex items-center space-x-2 px-3 py-1 text-sm font-medium rounded-md bg-green-600 hover:bg-green-700 transition-colors"><PlusIcon size={16}/><span>Přidat</span></button>
                </div>
                <div className="space-y-3">
                    {tariff.flatRates.map(rate => (
                        <div key={rate.id} className="grid grid-cols-10 gap-3 items-center">
                            <div className="col-span-6">
                                <label htmlFor={`rate-name-${rate.id}`} className="sr-only">Název trasy</label>
                                <input 
                                    type="text" 
                                    id={`rate-name-${rate.id}`}
                                    value={rate.name} 
                                    onChange={(e) => handleFlatRateChange(rate.id, 'name', e.target.value)}
                                    placeholder="Popis trasy (např. V rámci Mikulova)"
                            ...
                            <div className="col-span-3">
                                 <label htmlFor={`rate-price-${rate.id}`} className="sr-only">Cena</label>
                                 <div className="relative">
                                    <input 
                                        type="number" 
                            ...
                            <div className="col-span-1">
                                <button onClick={() => deleteFlatRate(rate.id)} className="p-2 text-red-500 hover:text-red-400 rounded-full hover:bg-red-500/10" aria-label="Smazat paušální sazbu"><CloseIcon size={18}/></button>
                            </div>
                        </div>
                    ))}
                    {tariff.flatRates.length === 0 && (
                        <p className="text-gray-400 text-sm text-center py-2">Nejsou definovány žádné paušální sazby.</p>
                    )}
                </div>
            </section>
        </div>
        ...
        <div className="flex justify-between items-center p-6 bg-slate-900/50 border-t border-slate-700 rounded-b-lg">
            <button type="button" onClick={handleReset} className="flex items-center space-x-2 px-4 py-2 text-sm font-medium rounded-md bg-slate-600 text-gray-200 hover:bg-slate-500 transition-colors focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 focus:ring-offset-slate-800"><UndoIcon size={16}/><span>Obnovit výchozí</span></button>
            <div className="space-x-3">
                ...
                <button type="button" onClick={handleSave} className="px-4 py-2 text-sm font-medium rounded-md shadow-sm text-white bg-amber-600 hover:bg-amber-700 transition-colors focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-2 focus:ring-offset-slate-800">Uložit tarif</button>
            </div>
        </div>
      </div>
    </div>
