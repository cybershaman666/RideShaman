...
export const DispatchFormComponent: React.FC<DispatchFormProps> = ({ onSubmit, onSchedule, isLoading, rideHistory, cooldownTime, onRoutePreview }) => {
  ...
  const InputField: React.FC<{ label: string, id: string, value: string | number, onChange: (e: React.ChangeEvent<HTMLInputElement>) => void, type?: string, error?: string }> = ({label, id, value, onChange, type='text', error}) => (
    <div>
      ...
      <input type={type} id={id} name={id} value={value} onChange={onChange} className={`w-full bg-slate-700 border ${error ? 'border-red-500' : 'border-slate-600'} rounded-md shadow-sm py-1 px-3 text-white focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500`} />
      {error && <p className="mt-1 text-xs text-red-400">{error}</p>}
    </div>
  );
    ...
    <div className="bg-slate-800 p-2 rounded-lg shadow-2xl flex flex-col h-full">
        ...
        <div className="flex-grow space-y-2 overflow-y-auto pr-2">
                ...
                <div>
                    <div className="flex justify-between items-center mb-1">
                        ...
                        <div className="flex items-center">
                            <input type="checkbox" id="isScheduled" checked={isScheduled} onChange={(e) => handleScheduleToggle(e.target.checked)} className="h-4 w-4 rounded bg-slate-700 border-slate-600 text-amber-500 focus:ring-amber-600 focus:ring-offset-slate-800 cursor-pointer"/>
                            <label htmlFor="isScheduled" className="ml-2 text-sm text-gray-300 cursor-pointer">{t('dispatch.schedule')}</label>
                        </div>
                    </div>

                    {isScheduled ? (
                        <input type="datetime-local" id="pickupTime" name="pickupTime" value={pickupTime === 'ihned' ? formatDateForPicker(new Date()) : pickupTime} onChange={(e) => setPickupTime(e.target.value)} min={formatDateForPicker(new Date())} className={`w-full bg-slate-700 border ${errors.pickupTime ? 'border-red-500' : 'border-slate-600'} rounded-md shadow-sm py-1 px-3 text-white focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500`}/>
                    ) : (
            ...
            <div>
                ...
                <textarea id="notes" name="notes" value={notes} onChange={(e) => setNotes(e.target.value)} rows={1} className="w-full bg-slate-700 border border-slate-600 rounded-md shadow-sm py-1 px-3 text-white focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500" placeholder={t('dispatch.notesPlaceholder')}></textarea>
            </div>
        </div>
